...
export async function fetchFileTreeRaw() {
    ...
    console.log('[SERVICE] Making file tree request with token');
...
export async function fetchFileContentRaw(cleanPath: string) {
    ...
    console.log('[SERVICE] Making file content request with token');
    const response = await fetch(`${BASE_URL}/files/content?${params.toString()}`, {
        headers: {
            'Authorization': `Bearer ${token}`
        }  
    });
