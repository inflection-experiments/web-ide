<script lang="ts">
  ...
  // FIX: Use 'let' for $props destructuring
  let { 
  ...
  // Derived values use 'const' (they're read-only)
  ...
  function manualSave() {
    if (!browser) return;
    if (!selectedFile || code === undefined || code === null) return;
    if (saveTimeout) {
    ...
    // FIX: Now we can assign because selectedFileContent is bindable
    ...
    return new Promise((resolve, reject) => {
      ...
      loaderScript.onload = () => {
        ...
        require.config({ 
          ...
          'vs/nls': { availableLanguages: { '*': undefined } }
        });
  ...
  $effect(() => {
    ...
    loadMonacoFromCDN().then(_monaco => {
      monaco = _monaco;
      if (!monaco) return;
      ...
      editor = monaco.editor.create(editorContainer, {
        ...
        scrollbar: {
          ...
          verticalScrollbarSize: 0, horizontalScrollbarSize: 0,
