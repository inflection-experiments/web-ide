<script lang="ts">
  ...
  let currentView = $state<'login' | 'register' | 'transitioning'>('login');
  let targetView = $state<'login' | 'register'>('login');
  ...
  function handleSwitch(newView: 'login' | 'register') {
    console.log('[DEBUG] Switching auth view to:', newView);
